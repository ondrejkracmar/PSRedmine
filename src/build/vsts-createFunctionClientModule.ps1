...
$parentModule = 'PSRedmine'
if (-not $ModuleName) { $ModuleName = 'PSRedmine.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSRedmine'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSRedmine.psd1"
$prereqHash = @{
